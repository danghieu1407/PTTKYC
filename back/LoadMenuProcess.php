...
if (isset($_GET["Danhmuc"]) && !empty($_GET["Danhmuc"])) {
    ...
    $stmt->bind_param("s", $_GET["Danhmuc"]);
    $stmt->execute();
...
?>
    <div class="card">
        ...
        <div class="price">
            <span><b><?php echo $row["Price"] ?>VND</b></span>
        </div>
        <div class="description">
            <h2 style="margin-left: 10px;"><b><?php echo $row["ProductName"] ?></b></h2>
            <p style="margin-left: 10px;"><?php echo $row["Description"] ?></p>
        </div>
        <form action="back/AddProductToCartProcess.php" method="POST">
            <input type="hidden" name="ProductID" value="<?php echo $row["ProductID"] ?>">
            <button type="submit" id="AddToCart">Thêm vào giỏ hàng</button>
        </form>
    </div>
<?php } ?>